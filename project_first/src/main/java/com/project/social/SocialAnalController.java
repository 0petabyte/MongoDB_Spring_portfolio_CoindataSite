...
@Controller
public class SocialAnalController {

	
	@Autowired
	private SocialAnalDao dao;
	
	
	@RequestMapping(value="/chartsocial.do", method = RequestMethod.GET)
	public ModelAndView list(@RequestParam(value="pageNum",defaultValue="1") int currentPage)
	{
		ModelAndView model=new ModelAndView();
		List<SocialAnalDto> list=dao.findList();
		int endPage;
		...
		model.setViewName("/1/content/chartsocial");
		
		
		List<BitfinexVolumeDto> list3=dao.findList2();
		
		model.addObject("list3",list3);
		
		
		List<ChartbarDto> list2=dao.findList1();
