...
<!-- 						<select class="form-control" id="whatday"
							name="whatday">
								...
								<option value="totalnews">전체NEWS</option>
										...
										<select class="form-control"onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);">
    <option value="" selected disabled hidden>Choose here</option>
    <option selected value="chartnewstoday.do" class="form-control">오늘뉴스</option>
    <option value="chartnewstotal.do" class="form-control">전체뉴스</option>
</select></td>
...
function googleTranslateElementInit() {
  new google.translate.TranslateElement({pageLanguage: 'en', includedLanguages: 'ko', layout: google.translate.TranslateElement.InlineLayout.SIMPLE,autoDisplay: false}, 'google_translate_element');
		...
		<c:if test="${count>0}">
				...
				<b>   오늘은 <span style="color:red;">${count}</span>개의 뉴스가 있습니다.</b>
...
%></p>
					<div class="table-responsive">
						<table class="table table-hover" id="dataTables-example">
							...
							<tbody>
							<c:forEach var="chartnews" begin="${startNum}" end="${endNum}" items="${list}" >
								<tr style="cursor: pointer;">
													<td class="testline" align="center">${no}</td>	
								<c:set var="no" value="${no-1}"/>
									<td class="testline" align="center">
									${chartnews.title}</td>
									<td class="testline" align="center"><i class="la la-sort-desc"></i></td>

									<td class="testline" align="center">${chartnews.newstime}</td>
									<td class="testline" align="center"><a href="${chartnews.newsurl}" target="_blank"><i class="la la-internet-explorer"></i>Link</a></td>
									
				

								</tr>
								<tr style="display: none;" id="scontentsub" class="subshow" >
<td colspan="4" align="center" > <br>${chartnews.content}</td>
	   	...
	   	<tr>
	   		<td align="center">
				...
				<%@ page import="com.project.*"%>
	   				 ...
	   				 <li class="page-item" ><a class="page-link" href="chartnewstoday.do?${nowurl}&pageNum=${startPage-1}">Previous</a></li>   				 
	   			</c:if>
	   			<c:forEach var="pg" begin="${startPage}" end="${endPage}">
	   				<c:if test="${currentPage==pg}">
	   					<li class="page-item"><a class="page-link" href="chartnewstoday.do?${nowurl}&pageNum=${pg}" style="color:red;">${pg}</a></li>
	   				</c:if>
	   				<c:if test="${currentPage!=pg}">
	   					<li class="page-item"><a class="page-link" href="chartnewstoday.do?${nowurl}&pageNum=${pg}" style="color:black;">${pg}</a></li>
	   				</c:if>
	   			</c:forEach>
	   			<c:if test="${endPage<totalPage}">
	   				<li class="page-item"><a class="page-link" href="chartnewstoday.do?${nowurl}&pageNum=${endPage+1}">Next</a></li>   
	   			</c:if>
	   			</ul>
